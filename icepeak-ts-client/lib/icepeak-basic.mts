...
function parseMessageEvent(event : ws.MessageEvent): Either<unknown, IncomingPayload> {
  ...
  case "update":
    if (!("path" in json && typeof json.path == 'string' && "value" in json))
      return fail(json)
    const update : ValueUpdate = { type: "update", path: json.path, value: json.value };
  ...
  case "subscribe":
    ...
    switch (json.code) {
    case 200:
      if (!("message" in json && typeof json.message == 'string')) return fail(json);
      if (!("paths" in json && Array.isArray(json.paths))) return fail(json);
